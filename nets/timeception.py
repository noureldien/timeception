...
from nets.keras_layers import ReshapeLayer, TransposeLayer, DepthwiseConv1DLayer, DepthwiseConv1DLayer
from nets.keras_layers import MaxLayer, AverageLayer, SumLayer, ExpandDimsLayer, SqueezeLayer, ChannelShuffleLayer

# region Timeception as Layers

def timeception_layers(tensor, n_layers, n_groups, is_dilated):
...
def __grouped_convolutions(tensor_input, n_groups, n_channels_per_branch, is_dilated, layer_num):
    ...
    # shuffle channels
    tensor = ChannelShuffleLayer(n_groups)(tensor)
...
def __temporal_convolutional_block(tensor, n_channels_per_branch, kernel_sizes, dilation_rates, layer_num, group_num):
    ...
    t_0 = Conv3D(n_channels_per_branch, kernel_size=(1, 1, 1), padding='same', name='conv_b0_g%d_tc%d' % (group_num, layer_num))(tensor)
    t_0 = BatchNormalization(name='bn_b0_g%d_tc%d' % (group_num, layer_num))(t_0)
...
class Timeception(Model):
    ...
    def __define_temporal_convolutional_block(self, n_channels_per_branch, kernel_sizes, dilation_rates, layer_num, group_num):
        ...
        # branch 1: dimension reduction only and no temporal conv
        layer_name = 'conv_b0_g%d_tc%d' % (group_num, layer_num)
        layer = Conv3D(n_channels_per_branch, kernel_size=(1, 1, 1), padding='same', name=layer_name)
        setattr(self, layer_name, layer)
        layer_name = 'bn_b0_g%d_tc%d' % (group_num, layer_num)
